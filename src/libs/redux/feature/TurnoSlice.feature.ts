...
const ChangeSelectShiftDateStaff = (
  state: ChangeSelectShiftDateStaff,
  start_date: Date,
  end_date: Date
) => {
    ...
    state.events = [
      {
        ...
        date: start_date.toISOString().substring(0, 10),
...
const ChangeSelectShiftDateSupportStaff = (
  state: ChangeSelectShiftDateStaff,
  start_date: Date,
  end_date: Date
) => {
...
export const turnoSlice = createSlice({
  ...
  reducers: {
    ChangeViewActionsShift: (
      state,
      actions: { type?: string; payload: { shift?: Shift } }
    ) => {
    ...
    SeeTurn: (state, actions) => {
      ...
      state.events = [
        ...
        {
          title: "Turno apoyo",
          date: support_start_date,
          end: support_end_date,
          id: 2,
        },
      ];
    },
    ChangeShiftInformation: (
      state,
      actions: {
        payload: { name: keyof Shift | string; value: string | Date | number };
        type: string;
      }
    ) => {
    ...
    SelectShiftDates: (
      state,
      action: { payload: { start_date: Date; end_date: Date }; type: string }
    ) => {
