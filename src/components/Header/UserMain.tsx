const UserMain = () => {
  ...
  return (
    <div className="fixed mt-[3.4rem] right-2 w-[220px]  p-2 bg-[#ffff] border-[1px] border-[#f5ededf8]  shadow-xl rounded-lg flex flex-col gap-2">
      ...
      {items.map((item, index) => {
        return (
          <ul
            key={index}
            className="flex gap-1 place-items-center hover:bg-[#81858133] p-1 rounded-sm cursor-pointer mx-1"
          >
            <li className="h-[1.1rem] ">{item.icon}</li>
            <span className="text-sm text-[#4b4b4b] font-[400]">
              {item.text}
            </span>
          </ul>
        );
